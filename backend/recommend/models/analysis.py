...
class General(object):
    ...
    def GetWatchedMovies():
        watched_ids = RT.UserVotes.objects.all().values_list('Movie_ID', flat=True)
        movies_ls = MT.MasterMovie.objects.filter(Movie_ID__in=watched_ids
                                        ).order_by('-ScoreImdb').values()

        # hack until FK is up 

        votes_ls = RT.UserVotes.objects.all().values()
    ...
    def GetSyntheticScores():
        # this is just for data science page, so keep a constant figure
...
class FeatureEngineer(object):
    ...
    def GetSingularMaximums():
        ...
        total_obs = MT.UserVotes.objects.all().count()
    ...
    def GetTargetNFeatures():
        ...
        target_ls = MT.UserVotes.objects.values()
    ...
    def CreateSyntheticVotes():
        ...
        RD.seed(666)
        number_votes = 900
        RT.UserVotes.objects.all().delete()
        ...
        data_obj_ls = [RT.UserVotes(**r) for r in vote_ls]
        RT.UserVotes.objects.bulk_create(data_obj_ls)
